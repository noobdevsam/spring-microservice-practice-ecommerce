...
class CustomerControllerTest {
    ...
    void createCustomer_ReturnsId() throws Exception {
        when(
                customerService.createCustomer(any(CustomerRequestDTO.class))
        ).thenReturn("1");
    ...
    void updateCustomer_ReturnsAccepted() throws Exception {
        doNothing().when(
                customerService).updateCustomer(any(CustomerRequestDTO.class)
        );
    ...
    void existsById_ReturnsTrue() throws Exception {
        when(
                customerService.existsCustomerById("1")
        ).thenReturn(true);

        mockMvc.perform(
                        get("/api/v1/customers/exists/1")
                )
                .andExpect(status().isOk())
